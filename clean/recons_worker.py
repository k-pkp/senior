...
def main():
    ...
    # Poisson — try multiple depths, running each in a sub-subprocess
    # so crashes don't kill the worker. Write the prepared pcd to a temp file.
    import tempfile
...
"""
            ...
            if r.returncode == 0 and os.path.exists(mesh_out):
                ...
                print(f"Poisson: depth={d}, {len(mesh.vertices):,} verts, {len(mesh.triangles):,} faces")
                break
    ...
    # This gives MeshFix a single surface with clean boundaries to close
    ...
    try:
        ...
        if len(cluster_n_tris) > 1:
            ...
            print(f"Largest component: kept {len(mesh.triangles):,}/{before:,} faces "
                  f"({len(mesh.triangles)/before*100:.1f}%), {len(cluster_n_tris)} components total")
    except Exception as e:
