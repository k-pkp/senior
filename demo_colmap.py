...
# Configure CUDA settings (safe no-op when CUDA is unavailable)
...
from vggt.utils.device import get_device, get_autocast_dtype, autocast_on, empty_cache
...
def run_VGGT(model, images, device, resolution=518):
    # images: [B, 3, H, W]
...
def demo_fn(args):
    ...
    extrinsic, intrinsic, depth_map, depth_conf = run_VGGT(model, images, device, vggt_fixed_resolution)
