...
from recons import reconstruct_multiple_objects, reconstruct_single_object, merge_meshes, reconstruct_mesh
from vggt.models.vggt import VGGT
from vggt.utils.load_fn import load_and_preprocess_images
from vggt.utils.pose_enc import pose_encoding_to_extri_intri
from vggt.utils.geometry import unproject_depth_map_to_point_map

from vggt.utils.device import get_device, get_autocast_dtype, autocast_on, empty_cache, aggressive_cleanup, is_mps
...
def run_model(target_dir, model) -> dict:
    ...
    # Auto-limit frames on MPS to avoid OOM (global attention is O(n^2) over frames)
    MPS_MAX_FRAMES = 6
    if is_mps(device) and len(image_names) > MPS_MAX_FRAMES:
        import numpy as _np
        indices = _np.linspace(0, len(image_names) - 1, MPS_MAX_FRAMES, dtype=int)
        image_names = [image_names[i] for i in sorted(set(indices))]
        print(f"  MPS: limited to {len(image_names)} frames to fit in memory")
...
# -------------------------------------------------------------------------
def load_baam_folder():
    """
    Load images from the ./baam/ folder directly.
    Returns (target_dir, image_paths) compatible with the upload handler.
    """
    baam_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "baam")
    if not os.path.isdir(baam_dir):
        return None, []

    image_exts = (".png", ".jpg", ".jpeg", ".bmp", ".tiff", ".webp")
    baam_images = sorted([
        os.path.join(baam_dir, f) for f in os.listdir(baam_dir)
    ...
    target_dir = f"input_images_baam_{timestamp}"
    target_dir_images = os.path.join(target_dir, "images")
    os.makedirs(target_dir_images, exist_ok=True)

    image_paths = []
    for src in baam_images:
        dst = os.path.join(target_dir_images, os.path.basename(src))
        shutil.copy(src, dst)
        image_paths.append(dst)

    print(f"Loaded {len(image_paths)} images from {baam_dir}")
    return target_dir, image_paths


def on_load_baam():
    """Gradio callback: load ./baam/ images and return UI updates."""
    target_dir, image_paths = load_baam_folder()
    if not image_paths:
        return (
            None,
            "None",
            None,
            "No images found in ./baam/ folder.",
            "Pipeline idle.",
            None,
            None,
            {"next_step": -1},
    ...
    return (
        ...
        target_dir,
        image_paths,
        f"Loaded {len(image_paths)} images from ./baam/. Click 'Reconstruct' to begin.",
...
def run_volume_pipeline(
    ...
    """Run clean/reconstruct pipeline and return final status + scene mesh path."""
    del cube_size_cm
    ...
    cleaned_ply_path = clean_and_extract_objects(
    ...
    mesh_ply_path, _ = reconstruct_multiple_objects(
        input_paths=input_objects,
        output_folder="output_mesh",
        base_name="scene"
    )

    status_msg = f"Pipeline finished. Reconstructed mesh: {mesh_ply_path}"
    return status_msg, mesh_ply_path
...
def continue_pipeline_step(
    ...
    try:
        ...
        if next_step == 1:
            ...
            cleaned_object_paths = clean_and_extract_objects(
                input_path=input_ply_path,
                output_folder=clean_input_dir,
                k=2,
                visualize=False
            )
            pipeline_state["cleaned_object_paths"] = cleaned_object_paths
            pipeline_state["next_step"] = 2
            msg = (
                f"Step 2 done. Input used: {input_ply_path}\n"
                f"Output objects: {cleaned_object_paths}\n"
                "Click Continue Step."
            )
            first_object = cleaned_object_paths[0] if cleaned_object_paths else None
            return pipeline_state, msg, first_object, None, "Step 2 completed."

        if next_step == 2:
            cleaned_object_paths = pipeline_state.get("cleaned_object_paths")
            if not cleaned_object_paths or not all(os.path.exists(path) for path in cleaned_object_paths):
                raise FileNotFoundError("Step 2 output missing. Click Redo to rerun Step 2.")
            mesh_ply_path, _ = reconstruct_multiple_objects(
                input_paths=cleaned_object_paths,
            ...
            msg = (
                f"Step 3 done. Input used: {cleaned_object_paths}\n"
...
def gradio_demo(
    ...
    volume_msg = "GLB saved. Click Continue Step to run Step 1 (export -> clean -> reconstruct)."
    mesh_ply_path = None
...
with gr.Blocks(
    ...
    <ol>
        ...
        <li><strong>Continue Steps:</strong> Click "Continue Step" to run stages in order: export PLY, clean PLY, reconstruct mesh.</li>
        <li><strong>Redo If Needed:</strong> Click "Redo Step" to rerun the latest completed stage if a step fails.</li>
        <li><strong>Read Results:</strong> View pipeline status in the result panel and download PLY output from the file panel.</li>
    </ol>
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            load_baam_btn = gr.Button("Load ./baam/ images", variant="secondary", size="sm")
        ...
        with gr.Column(scale=4):
            ...
            with gr.Row():
                volume_output = gr.Markdown("Pipeline idle.")
            ...
            with gr.Row():
                conf_thres = gr.Slider(minimum=0, maximum=100, value=45, step=0.1, label="Confidence Threshold (%)")
