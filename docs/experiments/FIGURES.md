...
| `mls_plane_bias_check.png` | **built 2026-08-24** | tests the closed form for plane-MLS's inward bias, `Δ = H·r²/4`, against the actual difference between the two fits on 4000 neighbourhoods. Correlation 0.988, slope 0.842. Pointwise only — it does **not** reproduce the aggregate −1.36% slice-area figure, which is measured on the pipeline's own scales. See `docs/mls_explained.md` §9. |
